...
Expr eval_under_env(Expr e, std::map<string, Expr> env) {
    return std::visit(overload {
        ...
        [&](box<struct IsAUnit>& isa) { 
            ...
            if (is<AUnit>(e1)) {
                return eval_under_env(Int(1),env);
            } else {
                return eval_under_env(Int(0),env);
            }
            //return e;
        },
        ...
        [&](box<struct MLet>& l) {
            env.insert_or_assign(l->varName, eval_under_env(l->e1, env));
            return eval_under_env(l->e2, env);
            //return e;
        },
        [&](box<struct Fun>& f) {
        ...
        [&](box<struct APair>& ap) {
            ...
            //struct APair pair = APair(e1, e2);
            return Expr(APair(e1, e2));
            //return e;
        },
        [&](box<struct Fst>& fst) {
            Expr pair = eval_under_env(fst->e, env);
            if (is<APair>(pair)) {
                struct APair apair = *std::get<box<struct APair>>(pair);
                return eval_under_env(apair.e1,env);
            } else {
                throw std::runtime_error("Fst: Not a pair");
            }
            // return e;
        },
        [&](box<struct Snd>& snd) {
            Expr pair = eval_under_env(snd->e, env);
            if (is<APair>(pair)) {
                struct APair apair = *std::get<box<struct APair>>(pair);
                return eval_under_env(apair.e2,env);
            } else {
                throw std::runtime_error("Fst: Not a pair");
            }
            return e;
        ...
        [&](box<struct Call>& call) {
            Expr fun_expr = eval_under_env(call->funExpr, env); // closure
            if (is<Closure>(fun_expr)) {
                struct Closure closure = *std::get<box<struct Closure>>(fun_expr);
                closure.env.insert_or_assign(closure.f.argName, eval_under_env(call->actual, env));
...
Expr MuplMap() { // body
    ...
    //      muplrec /* UPDATED */
    //    end
    Expr f = Fun("muplrec", "lst", IfAUnit(Var("lst"), AUnit(), APair(Call(Var("fun_arg"), Fst(Var("lst"))), Call(Var("muplrec"), Snd(Var("lst"))))));

    return Expr(Fun("","fun_arg", f));

    //Expr f = Fun("add1", "x", Add(Var("x"), Int(1)))
}
...
Expr MuplMapAddN() {
    ...
    // I lst
    Expr map = MuplMap();
    struct Fun f = Fun("", "x", Add(Var("x"), Var("I")));

    return Fun("","I",Call(map,f));
    //return f;

}

//Write a C++ function ToMuplList that takes a List<Expr> and produces an analogous MUPL list with the same elements in the same order.
Expr ToMuplList(const List<Expr>& cppList) {
    
  Expr muplList = AUnit(); // MUPL 리스트를 나타내는 MUPLList 객체를 AUnit()으로 초기화합니다.

    // C++ List의 각 요소에 대해 MUPL 리스트에 추가합니다.
    for (auto iter = cppList; !iter.isEmpty(); iter = iter.tail()) {
        muplList = APair(iter.head(), muplList);
    }
...
List<Expr> FromMupllist(const Expr& mupllist){
    List<Expr> cppList = List<Expr>(); // C++ List를 나타내는 List<Expr> 객체를 초기화합니다.
    
    // MUPL 리스트의 각 요소에 대해 C++ List에 추가합니다.
    Expr temp_mupl = mupllist;
    while (is<APair>(temp_mupl)) {
        APair ap = *std::get<box<struct APair>>(temp_mupl);
        cppList.cons(ap.e1);
        temp_mupl = ap.e2;
    }
